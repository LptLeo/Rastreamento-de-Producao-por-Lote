...
async function seed() {
  try {
    console.log(
      `[seed] 🚀 Iniciando geração de dados no host: ${process.env.DB_HOST || 'localhost'}`,
    );
    ...
    await AppDataSource.transaction(async (transactionalEntityManager) => {
      ...
      const gestor = await upsertUsuario(
        'Admin Gestor',
        'gestor@lotepim.com',
        PerfilUsuario.GESTOR,
      );
      const operador = await upsertUsuario(
        'Carlos Operador',
        'operador@lotepim.com',
        PerfilUsuario.OPERADOR,
      );
      const inspetor = await upsertUsuario(
        'Ana Inspetora',
        'inspetor@lotepim.com',
        PerfilUsuario.INSPETOR,
      );
      const op2 = await upsertUsuario(
        'Marcos Operador 2',
        'operador2@lotepim.com',
        PerfilUsuario.OPERADOR,
      );
      const ins2 = await upsertUsuario(
        'Julia Inspetora 2',
        'inspetor2@lotepim.com',
        PerfilUsuario.INSPETOR,
      );
      ...
      for (const d of mpDados) {
        const m = await mpRepo.findOneBy({ sku_interno: d.sku_interno });
        if (!m) {
      ...
      for (let idx = 1; idx <= 25; idx++) {
        ...
        const p = await produtoRepo.findOneBy({ sku });
        if (!p) {
          produtosToSave.push(
            produtoRepo.create({
      ...
      if (produtosToSave.length > 0) {
        ...
        produtosSalvos = await produtoRepo.find();
      }
      ...
      if (countLotes === 0) {
        ...
        const prodsComReceita = (
          await produtoRepo.find({ relations: ['receita', 'receita.materiaPrima'] })
        ).filter((p) => p.receita && p.receita.length > 0);

        const todosProdutos = await produtoRepo.find();
        const todosEstoques = await estoqueRepo.find({
        ...
        for (let i = 0; i < SEED_TOTAL_LOTES; i++) {
          ...
          const taxaReprovacaoReal =
            SEED_REPROV_BASE + (Math.random() * 2 * SEED_REPROV_VAR - SEED_REPROV_VAR);
          ...
          const l = loteRepo.create({
            ...
            aberto_em: dataProd,
            encerrado_em:
              status === LoteStatus.EM_PRODUCAO ? null : new Date(dataProd.getTime() + 120000),
          });
        ...
        for (let i = 0; i < lotesSalvos.length; i++) {
          ...
          if (data.p.receita) {
            for (const item of data.p.receita) {
              const estoque = todosEstoques.find(
                (e) => e.materiaPrima.id === item.materiaPrima.id && e.quantidade_atual > 0,
              );
              ...
              if (estoque) {
                ...
                estoque.quantidade_atual = Math.max(
                  0,
                  Number(estoque.quantidade_atual) - qtdConsumo,
                );
              }
            }
          }
          ...
          if (data.status !== LoteStatus.EM_PRODUCAO) {
            const qtdRep =
              data.status === LoteStatus.REPROVADO
                ? rand(Math.floor(data.qtdPlanj * 0.2), data.qtdPlanj)
                : data.status === LoteStatus.APROVADO_RESTRICAO
                  ? rand(1, 10)
                  : 0;

            const resultado =
              data.status === LoteStatus.APROVADO
                ? ResultadoInspecao.APROVADO
                : data.status === LoteStatus.REPROVADO
                  ? ResultadoInspecao.REPROVADO
                  : ResultadoInspecao.APROVADO_RESTRICAO;

            inspecoesToSave.push(
              inspecaoRepo.create({
                lote: loteSalvo!,
