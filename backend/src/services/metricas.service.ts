...
export class MetricasService {
  ...
  private getIntervalo(
    periodo: PeriodoDashboard,
    dataRef: Date,
  ): { atual: [Date, Date]; passado: [Date, Date] } {
    ...
    const [
      ...
      this.loteRepo
        ...
        .where('lote.aberto_em BETWEEN :inicio AND :fim', {
          inicio: intervaloUnidades.atual[0],
          fim: intervaloUnidades.atual[1],
        })
      ...
      this.loteRepo
        ...
        .select('COALESCE(SUM(lote.quantidade_planejada), 0)', 'total')
        .where('lote.aberto_em BETWEEN :inicio AND :fim', {
          inicio: intervaloUnidades.passado[0],
          fim: intervaloUnidades.passado[1],
        })
        .getRawOne<{ total: string }>(),
